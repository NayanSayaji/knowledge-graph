...
function nodeListItem(node: KnowledgeNode, directory: string) {
  ...
  const primaryLink = node.resources[0]
    ? ` · [${escapeTable(node.resources[0].title || node.resources[0].website)}](${node.resources[0].url})`
    : "";
  return `- ${nodeLink({ ...node, title: topicTitle }, directory)}${primaryLink}`;
}
